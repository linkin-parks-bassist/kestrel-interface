...
static const char *FNAME = "m_effect_settings.c";
...
int init_effect_settings_page(m_ui_page *page)
{
	...
	page->data_struct = m_alloc(sizeof(effect_settings_page_str));
...
int configure_effect_settings_page(m_ui_page *page, void *data)
{
	...
	snprintf(title_buf, 128, "%s Settings", m_effect_name(effect));
...
void band_control_value_changed_cb(lv_event_t *e)
{
	...
	#ifdef USE_TEENSY
	m_message msg = create_m_message(M_MESSAGE_SET_SETTING_VALUE, "ssss", setting->id.profile_id, setting->id.effect_id, setting->id.setting_id, value);
...
{
	M_PRINTF("create_effect_settings_page_ui\n");
	if (!page)
		return ERR_NULL_PTR;
	...
	M_PRINTF("create_effect_settings_page_ui done\n");
	return NO_ERROR;
}
