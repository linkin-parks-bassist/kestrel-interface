...
static const char *FNAME = "m_effect_view.c";

m_ui_page *create_effect_view_for(m_effect *effect)
{
	M_PRINTF("create_effect_view_for(effect = %p)\n", effect);
	...
	M_PRINTF("create_effect_view_for done\n");
	return page;
}

int init_effect_view(m_ui_page *page)
{
	//m_printf("Init effect view...\n");
	if (!page)
		return ERR_NULL_PTR;
	...
	m_effect_view_str *str = m_alloc(sizeof(m_effect_view_str));
...
void effect_view_enter_settings_cb(lv_event_t *e)
{
	...
	m_effect_view_str *str = (m_effect_view_str*)page->data_struct;
...
int configure_effect_view(m_ui_page *page, void *data)
{
	//m_printf("Conpfigure effect view... page = %p, data = %p\n", page, data);
	if (!page || !data)
	{
		if (page)
	...
	m_effect *effect = (m_effect*)data;
	
	page->panel->text = m_effect_name(effect);
	
	m_effect_view_str *str = page->data_struct;
...
int create_effect_view_ui(m_ui_page *page)
{
	M_PRINTF("create_effect_view_ui\n");
	if (!page)
		return ERR_NULL_PTR;
	...
	page->panel->text = m_effect_name(str->effect);
	...
	M_PRINTF("create_effect_view_ui done\n");
	return NO_ERROR;
}

int enter_effect_view(m_ui_page *page)
{
	#ifdef USE_TEENSY
...
int free_effect_view_ui(m_ui_page *page)
{
	if (!page)
		return ERR_NULL_PTR;
...
int free_effect_view(m_ui_page *page)
{
	if (!page)
		return ERR_NULL_PTR;
