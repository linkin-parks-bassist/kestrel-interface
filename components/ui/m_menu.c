...
int init_menu_item(m_menu_item *item)
{
	if (!item)
		return ERR_NULL_PTR;
...
int configure_menu_item(m_menu_item *item)
{
	if (!item)
		return ERR_NULL_PTR;
...
int delete_menu_item_ui(m_menu_item *item)
{
	if (!item)
		return ERR_NULL_PTR;
...
int free_menu_item(m_menu_item *item)
{
	if (item->text)
		m_free(item->text);
	if (item->desc)
...
int refresh_menu_item(m_menu_item *item)
{
	if (!item)
		return ERR_NULL_PTR;
...
m_menu_item *create_profile_listing_menu_item(char *text, m_profile *profile, m_ui_page *parent)
{
...
int profile_listing_menu_item_change_name(m_menu_item *item, char *name)
{
	if (!item)
		return ERR_NULL_PTR;
...
{
	m_menu_item *item = lv_timer_get_user_data(timer);
...
int create_menu_item_ui(m_menu_item *item, lv_obj_t *parent)
{
	if (!item)
		return ERR_NULL_PTR;
...
m_menu_item *create_pad_menu_item(int pad_height)
{
	m_menu_item *item = m_alloc(sizeof(m_menu_item));
...
m_menu_item *create_page_link_menu_item(char *text, m_ui_page *linked_page, m_ui_page *parent)
{
	m_menu_item *item = m_alloc(sizeof(m_menu_item));
...
m_menu_item *create_page_linindirect_k_menu_item(char *text, m_ui_page **linked_page, m_ui_page *parent)
{
	m_menu_item *item = m_alloc(sizeof(m_menu_item));
...
m_menu_item *create_parameter_widget_menu_item(m_parameter *param, m_ui_page *parent)
{
	if (!param)
		return NULL;
...
m_menu_item *create_danger_button_menu_item(void (*action_cb)(void* arg), void *arg, const char *text, m_ui_page *parent)
{
	if (!action_cb)
		return NULL;
...
int init_menu_page_str(m_menu_page_str *str)
{
	if (!str)
		return ERR_NULL_PTR;
...
int init_menu_page(m_ui_page *page)
{
	...
	m_menu_page_str *str = m_alloc(sizeof(m_menu_page_str));
...
int configure_menu_page(m_ui_page *page, void *data)
{
	...
	m_menu_page_str *str = (m_menu_page_str*)page->data_struct;
	...
	page->parent = data;
	m_menu_item_pll *current_item = str->items;
...
int menu_page_add_item(m_menu_page_str *str, m_menu_item *item)
{
	if (!str || !item)
		return ERR_NULL_PTR;
...
int menu_page_remove_item(m_ui_page *page, m_menu_item *item)
{
	if (!page)
		return ERR_NULL_PTR;
	...
	m_menu_item_pll *current_item = str->items;
	m_menu_item_pll *prev_item = NULL;
