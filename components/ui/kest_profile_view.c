...
static const char *FNAME = "kest_profile_view.c";

kest_ui_page *create_profile_view_for(kest_profile *profile)
{
	KEST_PRINTF("create_profile_view_for(profile = %p)\n", profile);
	if (!profile)
		return NULL;
	...
	KEST_PRINTF("Calling init_profile_view...\n");
	...
	KEST_PRINTF("Calling configure_profile_view...\n");
	...
	KEST_PRINTF("create_profile_view_for done\n");
	return page;
}
...
int profile_view_effect_moved_cb(kest_active_button *button)
{
	...
	#ifdef USE_TEENSY
	queue_msg_to_teensy(create_m_message(KEST_MESSAGE_MOVE_TRANSFORMER, "sss", effect->profile->id, effect->id, button->index));
	#endif
	...
	if (profile)
		kest_profile_move_effect(profile, button->index, button->prev_index);
	#endif
...
{
	KEST_PRINTF("profile_view_effect_delete_cb\n");
	if (!button)
		return ERR_NULL_PTR;
	...
	int ret_val = kest_profile_remove_effect(effect->profile, effect->id);
	
	if (ret_val != NO_ERROR)
		return ret_val;
	
	#ifdef USE_FPGA
	ret_val = kest_profile_if_active_update_fpga(effect->profile);
	#endif
	KEST_PRINTF("profile_view_effect_delete_cb done\n");
	return ret_val;
}

int init_profile_view(kest_ui_page *page)
{
	//kest_printf("init_profile_view...\n");
	if (!page)
		return ERR_NULL_PTR;
	...
	kest_profile_view_str *str = kest_alloc(sizeof(kest_profile_view_str));
...
static void save_button_cb(lv_event_t *e)
{
	...
	kest_profile_view_str *str = (kest_profile_view_str*)page->data_struct;
	
	if (!str)
		return;
	
	//kest_printf("Saving profile...\n");
	int ret_val;
	
	if ((ret_val = kest_profile_save(str->profile)) == NO_ERROR)
...
static void menu_button_cb(lv_event_t *e)
{
	...
	kest_profile *profile = str->profile;
	...
	if (profile->sequence)
	{
		KEST_PRINTF("profile has a sequence; enter sequence view %p\n", profile->sequence->view_page);
		enter_ui_page_backwards(profile->sequence->view_page);
	}
	else
	{
		KEST_PRINTF("profile has no sequence. enter ... whatevery. %p\n", page->parent);
...
int profile_view_save_name(kest_ui_page *page)
{
	...
	if (str->profile->name)
		kest_free(str->profile->name);
	
	str->profile->name = kest_strndup(new_name, PROFILE_NAME_MAX_LEN);
	...
	kest_profile_update_representations(str->profile);
...
{
	KEST_PRINTF("profile_view_play_button_cb\n");
	kest_ui_page *page = (kest_ui_page*)lv_event_get_user_data(e);
	...
	{
		kest_sequence_begin_at(str->profile->sequence, str->profile);
	}
	else
	{
...
{
	KEST_PRINTF("configure_profile_view(page = %p, data = %p)\n", page, data);
	if (!page || !data)
		return ERR_NULL_PTR;
	...
	kest_profile_view_str *str = page->data_struct;
	...
	kest_profile_add_representation(profile, &str->rep);
...
int create_profile_view_ui(kest_ui_page *page)
{
	...
	if (!profile->name)
	{
		KEST_PRINTF("create_profile_view_ui. profile->name = NULL\n");
		kest_profile_set_default_name_from_id(profile);
	}
	
	KEST_PRINTF("create_profile_view_ui. profile->name = %s\n", profile->name);
...
{
	KEST_PRINTF("enter_profile_view\n");
	if (!page)
		return ERR_NULL_PTR;
...
int profile_view_append_effect(kest_ui_page *page, kest_effect *effect)
{
	...
	KEST_PRINTF("profile_view_append_effect done; effect->view_page = %p\n", effect->view_page);
	return NO_ERROR;
}
...
{
	KEST_PRINTF("profile_view_rep_update. representer = %p, representee = %p\n", representer, representee);
	...
	kest_profile_view_str *str = (kest_profile_view_str*)page->data_struct;
	KEST_PRINTF("profile->active = %d,  str = %p, \n", profile->active, str);
	if (!str)
		return;
	...
	KEST_PRINTF("profile_view_rep_update done\n");
}
