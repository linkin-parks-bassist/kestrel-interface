...
m_menu_item *create_sequence_listing_menu_item(char *text, m_sequence *sequence, m_ui_page *parent)
{
	...
	if (sequence)
		m_sequence_add_menu_listing(sequence, item);
...
int sequence_listing_menu_item_change_name(m_menu_item *item, char *name)
{
	if (!item)
		return ERR_NULL_PTR;
...
int init_sequence_list(m_ui_page *page)
{
	...
	//m_printf("init_sequence_list\n");
	init_menu_page(page);
	
	page->configure = configure_sequence_list;
	
	//m_printf("init_sequence_list done\n");
	return NO_ERROR;
}
...
void sequence_list_add_cb(lv_event_t *e)
{
	...
	m_menu_page_str *str = page->data_struct;
	
	if (!str)
		return;
	
	m_sequence *new_sequence = m_context_add_sequence_rp(&global_cxt);
	...
	m_menu_item *new_listing = create_sequence_listing_menu_item(new_sequence->name, new_sequence, page);
...
void sequence_listing_delete_button_cb(lv_event_t *e)
{
	...
	m_sequence *sequence = (m_sequence*)item->data;
