...
int m_fpga_batch_append_block_instr(m_fpga_transfer_batch *batch, m_block *block, const m_eff_resource_report *res, int pos)
{
	if (!batch || !block)
		return ERR_NULL_PTR;
...
int m_fpga_batch_append_block_register_updates(m_fpga_transfer_batch *batch, m_block *block, m_expr_scope *scope, int pos)
{
	...
	if (block->reg_0.active)
	{
		if (block->reg_0.expr)
		{
			...
			m_fpga_batch_append(batch, COMMAND_UPDATE_BLOCK_REG_0);
			m_fpga_batch_append_block_number(batch, pos);
			m_fpga_batch_append_16(batch, s);
		}
	}
	...
	if (block->reg_1.active)
	{
		if (block->reg_1.expr)
		{
			...
			m_fpga_batch_append(batch, COMMAND_UPDATE_BLOCK_REG_1);
			m_fpga_batch_append_block_number(batch, pos);
			m_fpga_batch_append_16(batch, s);
		}
	}
...
int m_fpga_transfer_batch_append_effect_register_updates(m_fpga_transfer_batch *batch, m_effect_desc *eff, m_expr_scope *scope, int pos)
{
	...
	int i = 0;
	m_block_pll *current = eff->blocks;
...
int m_fpga_batch_print(m_fpga_transfer_batch seq)
{
	...
	M_PRINTF_("Reading out FPGA transfer batch %p (length %d)\n", seq.buf, n);
	
	if (!seq.buf)
	{
		M_PRINTF_("Buffer is NULL!\n");
	}
	
	if (n < 1)
	{
		M_PRINTF_("Batch has no bytes!\n");
	}
	...
	while (i < n)
	{
		...
		M_PRINTF_("\tByte %s%d: 0x%02X. ", (n > 9 && i < 10) ? " " : "", i, byte);
		...
		switch (state)
		{
			case 0: // expecting a command
				switch (byte)
				{
					case COMMAND_BEGIN_PROGRAM:
						M_PRINTF_("Command BEGIN_PROGRAM");
						break;
						
					case COMMAND_WRITE_BLOCK_INSTR:
						M_PRINTF_("Command WRITE_BLOCK_INSTR");
						state = 1;
						ret_state = 2;
						ctr = 0;
						instruction = 0;
					...
					case COMMAND_WRITE_BLOCK_REG_0:
						M_PRINTF_("Command WRITE_BLOCK_REG_0");
						state = 1;
						ret_state = 4;
						value = 0;
						ctr = 0;
					...
					case COMMAND_WRITE_BLOCK_REG_1:
						M_PRINTF_("Command WRITE_BLOCK_REG_1");
						state = 1;
						ret_state = 4;
						value = 0;
						ctr = 0;
					...
					case COMMAND_END_PROGRAM:
						M_PRINTF_("Command END_PROGRAM");
						break;

					case COMMAND_UPDATE_BLOCK_REG_0:
						M_PRINTF_("Command UPDATE_BLOCK_REG_0");
						state = 1;
						ret_state = 4;
						value = 0;
						ctr = 0;
					...
					case COMMAND_UPDATE_BLOCK_REG_1:
						M_PRINTF_("Command UPDATE_BLOCK_REG_1");
						state = 1;
						ret_state = 4;
						value = 0;
						ctr = 0;
					...
					case COMMAND_COMMIT_REG_UPDATES:
						M_PRINTF_("Command COMMIT_REG_UPDATES");
						break;

					case COMMAND_SET_INPUT_GAIN:
						M_PRINTF_("Command SET_INPUT_GAIN");
						state = 4;
						ret_state = 0;
						value = 0;
						ctr = 0;
					...
					case COMMAND_SET_OUTPUT_GAIN:
						M_PRINTF_("Command SET_OUTPUT_GAIN");
						state = 4;
						ret_state = 0;
						value = 0;
						ctr = 0;
					...
					case COMMAND_ALLOC_FILTER:
						M_PRINTF_("Command COMMAND_ALLOC_FILTER");
						state = 6;
						ret_state = 0;
						value = 0;
						ctr = 0;
					...
					case COMMAND_WRITE_FILTER_COEF:
						M_PRINTF_("Command COMMAND_WRITE_FILTER_COEF");
						state = 7;
						ret_state = 0;
						value = 0;
						ctr = 0;
			...
			case 4: // expecting a value
				if (ctr == M_FPGA_DATA_BYTES - 1)
				{
					...
					value = (value << 8) | byte;
					M_PRINTF_("Value: %s = %d = %f (in q%d.%d)", binary_print_16(value), value, (float)value / (powf(2.0, 15 - shift)), 1 + shift, 15 - shift);
				}
				else
				{
					value = (value << 8) | byte;
			...
			case 5: // expecting two 24-bit values. lol!
				...
				{
					value = (value << 8) | byte;
					M_PRINTF_("%s: %s = 0x%06x = %.02f", ctr_2 ? "Delay" : "Size", binary_print_24(value), value, (float)((uint32_t)value) / (powf(2.0, (15 - shift))));
			...
			case 6:
				if (ctr_2 == 0)
				{
					M_PRINTF_("Format: q%d.%d", 1 + byte, 17 - byte);
					shift = byte;
					ctr_2++;
				}
				else if (ctr_2 == 1)
				{
					...
					{
						value = (value << 8) | byte;
						M_PRINTF_("Feed-forward: %d", value);
			...
			case 7:
				...
				else
				{
					...
					if (ctr == 3)
					{
						...
						M_PRINTF_("Value: %.04f", ((float)signed_val) * pow(2, -(16 - shift)));
						state = 0;
					}
				}
				break;
