#ifndef KEST_DYNAMIC_ARRAY_H_
#define KEST_DYNAMIC_ARRAY_H_
...
\
    ...
    list->count = 0; \
    list->capacity = 0; \
    list->alloc = alloc ? *alloc : (kest_allocator){0}; \
\
    return NO_ERROR; \
} \
\
...
\
    list->entries = kest_allocator_alloc(&list->alloc, sizeof(X) * n); \
...
\
    if (!list->entries) \
    { \
        list->entries = kest_allocator_alloc(&list->alloc, sizeof(X) * cap_needed); \
        if (!list->entries) \
        { \
            list->count = 0; \
            list->capacity = 0; \
    ...
    { \
        X *new_array = kest_allocator_realloc(&list->alloc, list->entries, sizeof(X) * cap_needed); \
...
\
    if (list->entries) \
        kest_allocator_free(&list->alloc, list->entries); \
\
    list->entries = NULL; \
    list->count = 0; \
    list->capacity = 0; \
...
\
    ...
    { \
        for (int i = 0; i < list->count; i++) \
            if (list->entries[i]) kest_allocator_free(&list->alloc, list->entries[i]); \
    } \
\
    kest_allocator_free(&list->alloc, list->entries); \
\
