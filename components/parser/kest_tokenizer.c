...
int kest_token_ll_safe_append(kest_token_ll **list_ptr, char *x, int line, int index)
{
	...
	kest_token_ll *node = kest_alloc(sizeof(kest_token_ll));
	...
	if (*list_ptr)
	{
		kest_token_ll *current = *list_ptr;
...
int kest_token_ll_safe_aappend(kest_token_ll **list_ptr, char *x, int line, int index)
{
	...
	kest_token_ll *node = kest_parser_alloc(sizeof(kest_token_ll));
...
int kest_tokenize_content(kest_eff_parsing_state *ps)
{
	...
	{
		KEST_PRINTF("'%c' (%d), '%c' (%d), '%c' (%d), '%c' (%d)\n", buf[0], buf[0], buf[1], buf[1], buf[2], buf[2], buf[3], buf[3]);
		kest_parser_error_at_line(ps, 1, "Version string \"%s\" required at start of file; instead, file starts with \"%s\"", ver_str, buf);
		return ERR_BAD_ARGS;
	}

	kest_token_ll_safe_aappend(&ps->tokens, kest_parser_strndup(ver_str, 4), line, 0);
	...
	while (state != TOKENIZER_STATE_DONE)
	{
		...
		switch (policy)
		{
			...
			case TOKENIZER_POLICY_COMPLAIN:
				...
				kest_parser_error_at_line(ps, line, "Unexpected \"%s\"", buf);
				return ERR_BAD_ARGS;
		}
...
kest_token_ll *kest_token_span_to_ll(kest_token_ll *start, kest_token_ll *end)
{
	...
	kest_token_ll *res = NULL;
	kest_token_ll *current = start;
	
	while (current && current != end)
	{
		kest_token_ll_safe_aappend(&res, current->data, current->line, current->index);
		current = current->next;
	}
